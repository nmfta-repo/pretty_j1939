...
def test_basic_library_usage():
    ...
    db_path = os.path.join("tmp", "J1939db.json")
    if not os.path.exists(db_path):
        print(f"Skipping test: {db_path} not found")
        return

    # Initialize the describer
    describe = pretty_j1939.describe.get_describer(da_json=db_path)
...
def test_dm1_library_usage():
    print("Running test_dm1_library_usage...")
    db_path = os.path.join("tmp", "J1939db.json")
    describe = pretty_j1939.describe.get_describer(da_json=db_path if os.path.exists(db_path) else None)
...
def test_summary_generation_usage():
    ...
    renderer = pretty_j1939.render.HighPerformanceRenderer(da_describer=describer.da_describer)
