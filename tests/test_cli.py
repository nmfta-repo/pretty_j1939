...
def test_cli_new_rp1210_format():
    """Verify CLI parsing of the new RP1210-like candump format."""
    stdin_data = '(    795259.000000) RP1210:NORTDA32 0CF00400#087D7D0000FFF0FF                    ; {"PGN":"EEC1(61444)"}\n'
    db_path = os.path.join("pretty_j1939", "J1939db.json")
...
def test_cli_space_separated_format():
    """Verify CLI parsing of 'interface ID [length] data...' format."""
    stdin_data = "  can0  0CF00400   [8]  00 41 FF 20 48 14 00 F0\n"
    db_path = os.path.join("pretty_j1939", "J1939db.json")
...
def test_cli_space_separated_with_timestamp_format():
    """Verify CLI parsing of '(timestamp) interface ID [length] data...' format."""
    stdin_data = " (1612543138.000000)  can0  0CF00400   [8]  00 41 FF 20 48 14 00 F0\n"
    db_path = os.path.join("pretty_j1939", "J1939db.json")

    stdout, stderr, code = run_cli(
        ["-", "--da-json", db_path, "--json", "--candata", "candump"],
        stdin_content=stdin_data,
    )

    assert code == 0
    assert '"PGN":"EEC1(61444)"' in stdout
    assert "1612543138.000000" in stdout
