...
class DADescriber:
    def __init__(
        ...
        self._spn_cache = {} # Cache for (name, units, bitencoded, numerical, start, length, spn_obj)
    ...
    def get_spn_bytes(self, message_data_bitstring, spn, pgn, is_complete_message):
        ...
        spn_name, spn_units, is_num, is_bit, spn_start, spn_length, spn_obj = self._spn_cache[spn]
    ...
    def get_spn_value(
        ...
        # Drawback: code duplication and fragility; logic (validation, scaling) must be maintained in both 
        # this fast path and the general bit-level fallback path.
        if len(spn_start) == 1 and spn_start[0] % 8 == 0 and spn_length % 8 == 0:
            ...
            if start_byte + byte_len <= len(data_bytes):
                ...
                if byte_len == 1:
                    ...
                    value = cut_bytes[0] | (cut_bytes[1] << 8) | (cut_bytes[2] << 16) | (cut_bytes[3] << 24)
                else:
                ...
                if not is_bit:
                    ...
                    if validate:
                        if value < spn_obj["OperationalLow"] or value > spn_obj["OperationalHigh"]:
                            raise ValueError
                return value

        cut_data = self.get_spn_bytes(message_data_bitstring, spn, pgn, is_complete_message)
    ...
    def describe_message_data(
        ...
        if pgn == 65226: # DM1
            ...
            if len(data) >= 2:
                lamp_status = data[0]
                description["Malfunction Indicator Lamp Status"] = ["Off", "On", "Error", "Not Available"][(lamp_status >> 6) & 0x03]
                description["Red Stop Lamp Status"] = ["Off", "On", "Error", "Not Available"][(lamp_status >> 4) & 0x03]
                description["Amber Warning Lamp Status"] = ["Off", "On", "Error", "Not Available"][(lamp_status >> 2) & 0x03]
                description["Protect Lamp Status"] = ["Off", "On", "Error", "Not Available"][lamp_status & 0x03]
        ...
        if not spn_list:
             if (len(description) == 0 and not is_transport_pgn(pgn)) or self.include_raw_data:
        ...
        for spn in spn_list:
            ...
            if spn in self._spn_cache:
                ...
                self._spn_cache[spn] = (spn_name, spn_units, is_num, is_bit, spn_start, spn_length, spn_obj)
...
class J1939Describer:
    ...
    def __call__(
        self, message_data, message_id_uint: int
    ):
        ...
        if pgn == 59904: # Request
            ...
            if len(message_data) >= 24:
                req_pgn = message_data.bytes[0] | (message_data.bytes[1] << 8) | (message_data.bytes[2] << 16)
        ...
        is_transport_lower_layer_message = is_transport_message(message_id_uint) or \
                                           (message_id_uint & PF_MASK) == DIAG3_MASK
