...
# --- Constants ---
PRETTY_COL_OFFSET = 30
WRAP_INDENT = 4
BOUNCE_BUFFER_WIDTH = 10
...
class J1939Viewer:
    ...
    def _init_curses(self):
        ...
        curses.init_pair(1, get_theme_color("keys", "#00ffff"), -1)  # Keys
        curses.init_pair(2, get_theme_color("numbers", "#ffff00"), -1)  # Numbers
        curses.init_pair(3, get_theme_color("strings", "#ffffff"), -1)  # Default/Values
    ...
    def _format_value(self, key: str, val: Any) -> str:
        """Applies fixed-width limits to floating point numbers to prevent jitter."""
        s = str(val)
        if key == "Bytes":
            return s

        t = s.strip()
        if t and "." in t and any(c.isdigit() for c in t):
            if t[0].isdigit() or (t[0] in ("-", "+") and len(t) > 1 and t[1].isdigit()):
                return f"{s[:BOUNCE_BUFFER_WIDTH]:<{BOUNCE_BUFFER_WIDTH}}"
        return s
    ...
    def _iterate_pretty_fields(
        ...
        for k, v in state.description.items():
            ...
            yield k, val_str, is_changed, (k == "Bytes"), is_first
            is_first = False
    ...
    def _draw_header(self):
        ...
        self.stdscr.erase()
        text = f"{'Count':<8}{'dt':<8}{'ID':<12}Pretty"
