...
class J1939daConverter:
    ...
    def _get_column_indices(self, header_row):
        return {
            ...
            "acronym": self.get_any_header_column(
                header_row, ["ACRONYM", "PG_ACRONYM"]
            ),
            "pgn_label": self.get_any_header_column(
                header_row, ["PARAMETER_GROUP_LABEL", "PG_LABEL"]
            ),
            "pgn_data_length": self.get_any_header_column(
                header_row, ["PGN_DATA_LENGTH", "PG_DATA_LENGTH"]
            ),
            "transmission_rate": self.get_any_header_column(
                header_row, "TRANSMISSION_RATE"
            ),
            "spn_position_in_pgn": self.get_any_header_column(
                header_row, ["SPN_POSITION_IN_PGN", "SP_POSITION_IN_PG"]
            ),
            "spn_name": self.get_any_header_column(
            ...
            "resolution": self.get_any_header_column(
                header_row, ["RESOLUTION", "SCALING"]
            ),
            "spn_length": self.get_any_header_column(
                header_row, ["SPN_LENGTH", "SP_LENGTH"]
            ),
            "units": self.get_any_header_column(header_row, ["UNITS", "UNIT"]),
            "operational_range": self.get_any_header_column(
                header_row, "OPERATIONAL_RANGE"
            ),
            "spn_description": self.get_any_header_column(
    ...
    def _process_pgn(self, row, cols, pgn, j1939_pgn_db):
        ...
        pgn_object.update(
            {
                "Label": (
                    unidecode.unidecode(str(row[cols["acronym"]]))
                    if row[cols["acronym"]] is not None
                    else ""
                )
            }
        )
        pgn_object.update(
            {
                "Name": (
                    unidecode.unidecode(str(row[cols["pgn_label"]]))
                    if row[cols["pgn_label"]] is not None
                    else ""
                )
            }
        )
        ...
        pgn_object.update(
            {
                "Rate": (
                    unidecode.unidecode(str(row[cols["transmission_rate"]]))
                    if row[cols["transmission_rate"]] is not None
                    else ""
                )
            }
        )
    ...
    def _process_spn(self, row, cols, spn_label, j1939_spn_db):
        ...
        spn_resolution = (
            da_parsers.get_spn_resolution(str(row[cols["resolution"]]))
            if row[cols["resolution"]] is not None
            else 0
        )
        ...
        data_range = (
            unidecode.unidecode(str(row[cols["data_range"]]))
            if row[cols["data_range"]] is not None
            else ""
        )
        low, high = da_parsers.get_operational_hilo(data_range, spn_units, spn_length)

        spn_name = (
            unidecode.unidecode(str(row[cols["spn_name"]]))
            if row[cols["spn_name"]] is not None
            else ""
        )
        operational_range = (
            unidecode.unidecode(str(row[cols["operational_range"]]))
            if row[cols["operational_range"]] is not None
            else ""
        )
        spn_offset = (
            da_parsers.get_spn_offset(str(row[cols["offset"]]))
            if row[cols["offset"]] is not None
            else 0
        )
        ...
        if existing_spn is not None and not existing_spn == spn_object:
            print(
                ...
                % (spn_label, existing_spn, spn_object),
                file=sys.stderr,
            )
        else:
            ...
            if not spn == "" and not spn == "N/A" and spn is not None:
                ...
                try:
                    spn_position_contents = (
                        str(row[cols["spn_position_in_pgn"]])
                        if row[cols["spn_position_in_pgn"]] is not None
                        else ""
                    )
                    spn_startbit_inpgn, spn_order_inpgn = self._get_spn_startbit(
                        spn_label, spn_position_contents
                    )
                ...
                spn_description = (
                    unidecode.unidecode(str(row[cols["spn_description"]]))
                    if row[cols["spn_description"]] is not None
                    else ""
                )
                if spn_units == "bit" or da_parsers.is_spn_likely_bitmapped(
                    spn_description
                ):
                    bit_object = OrderedDict()
                    da_parsers.create_bit_object_from_description(
